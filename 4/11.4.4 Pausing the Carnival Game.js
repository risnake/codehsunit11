...
let weight;
let dy = -10;
let isPaused = false; // boolean variable to track if the game is paused
...
function main(){
    ...
    mouseClickMethod(togglePause); // add mouse click event listener
}
...
function launch() {
    if (!isPaused) { // only move the weight if the game is not paused
        checkCollision();
        pickColor();
        weight.move(0,dy);
    }
